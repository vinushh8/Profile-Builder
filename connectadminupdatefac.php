...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($conn->connect_error) {
        ...
        $stmt = $conn->prepare("UPDATE facultydetails SET name = ?, major = ?, academicachievements = ?, graduationdate = ?, certifications = ?, projects = ?, workexperience = ?, researchexperience = ?, technicalskills = ?, areaofinterest = ? WHERE username = ?");
        if (!$stmt) {
